...
const file = readFileSync('./scripts/ionx-merkle-root.json').toString('utf-8')
...
for (let i = 0; i < sortedAddresses.length; i += DESIRED_COHORT_SIZE) {
  ...
  writeFileSync(`./scripts/chunks/${sortedAddresses[i].toLowerCase()}.json`, JSON.stringify(sortedAddresses.slice(i, lastIndex + 1).reduce((claims, addr) => {
    claims[addr] = allClaims[addr]
    return claims
  }, <{ [address: string]: any }>{})))
}

writeFileSync(`./scripts/chunks/mapping.json`, JSON.stringify(addressChunks))
